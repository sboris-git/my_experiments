#not ready http://www.r-bloggers.com/computing-and-visualizing-pca-in-r/

library(devtools)
# install_github("ggbiplot", "vqv")
library(ggbiplot)

# ~/Dropbox/Students/Стельмащук/Stelmashchuk@student/UJ my measurements/Analysis/CSV/PCA_IV.csv
...
path.name.ext <- "~/Dropbox/Students/Стельмащук/Stelmashchuk@student/UJ my measurements/Analysis/CSVa/PCA_I-VIII.csv"
...
ir.pca <- prcomp(log.ir, center = TRUE, scale. = TRUE, na.action = na.exclude,)
...
g <- ggbiplot(ir.pca, obs.scale = 1, var.scale = 1, 
              groups = ir.species, ellipse = TRUE, circle = TRUE, circle.prob = 0.95, varname.size = 3, labels=CV.dat$dE_dt)
# http://www.sthda.com/english/wiki/ggplot2-scatter-plots-quick-start-guide-r-software-and-data-visualization
g <- g  + geom_point(aes(colour = factor(ir.species)), size = 4.5) +
          geom_point(colour="black", size = 2)# + 
          #geom_point(aes(shape = factor(ir.species)), size = 3.5)
...
print(g)
ggsave(filename = "PCA_I-VIII_experimental.png", plot = g, width = 6, height = 7)
